/**
 * * Authentication pages
 */
export { default as Login } from "./auth/Login";

/**
 * * Admin pages
 */
export { default as PermissionDenied } from "./admin/PermissionDenied";
export { default as Dashboard } from "./admin/Dashboard";
export { default as User } from "./admin/user/User";
export { default as AddUser } from "./admin/user/AddUser";
export { default as EditUser } from "./admin/user/EditUser";
export { default as Employee } from "./admin/employee/Employee";
export { default as AddEmployee } from "./admin/employee/AddEmployee";
export { default as EditEmployee } from "./admin/employee/EditEmployee";
export { default as Product } from "./admin/product/Product";
export { default as AddProduct } from "./admin/product/AddProduct";
export { default as EditProduct } from "./admin/product/EditProduct";
export { default as Order } from "./admin/order/Order";
